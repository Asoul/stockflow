...
class TraderRecorder():
    ...
    def getBuyAndSellSeries(self, value_series, trade_series):
        ...
        for i in range(len(value_series)):
            ...
            if trade_series[i] == 1:
                buy_series.append(value_series[i])
            else:
                buy_series.append(None)

            if trade_series[i] == -1:
                sell_series.append(value_series[i])
            else:
    ...
    def recordToPNG(self, result):
        ...
        buy_series, sell_series = self.getBuyAndSellSeries(result["Value Series"], result["Trade Series"])
